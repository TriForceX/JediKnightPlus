	...
	if ( charging )
	{
		if ( altFire )
		{
			...
			else if ((pm->cmd.serverTime - pm->ps->weaponChargeTime) < weaponData[pm->ps->weapon].altMaxCharge)
			{
				if (pm->ps->weaponChargeSubtractTime < pm->cmd.serverTime)
				{
					...
					if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] != INFINITE_VALUE) pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -= weaponData[pm->ps->weapon].altChargeSub;
					pm->ps->weaponChargeSubtractTime = pm->cmd.serverTime + weaponData[pm->ps->weapon].altChargeSubTime;
				}
			}
		}
		else
		{
			...
			else if ((pm->cmd.serverTime - pm->ps->weaponChargeTime) < weaponData[pm->ps->weapon].maxCharge)
			{
				...
				{
					// Tr!Force: [InfiniteAmmo] Ammo pmove check
					if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] != INFINITE_VALUE) pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -= weaponData[pm->ps->weapon].chargeSub;
					pm->ps->weaponChargeSubtractTime = pm->cmd.serverTime + weaponData[pm->ps->weapon].chargeSubTime;
				}
			}
		}
...
static void PM_Weapon( void )
{
	...
	{
		if ( pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != INFINITE_VALUE ) // Tr!Force: [InfiniteAmmo] Ammo pmove check
		{
