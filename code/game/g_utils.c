...
void TryUse( gentity_t *ent )
{
	...
	if ( trace.fraction == 1.0f || trace.entityNum < 1 )
	{
		...
		if (ent->client->ps.stats[JK_TWEAKS] & JK_USE_STAND) ent->client->pers.jkmodPers.buttonUseAnimValid = qfalse;
	...
	if ( ValidUseTarget( target ) )
	{
		...
		if ((ent->client->ps.stats[JK_TWEAKS] & JK_USE_STAND) && Q_stricmp("jkmod_chair_model", target->classname)) ent->client->pers.jkmodPers.buttonUseAnimValid = qtrue;
