...
void TurretClientRun(centity_t *ent)
{
	...
	else if (ent->currentState.fireflag && ent->bolt4 != ent->currentState.fireflag)
	{
		...
		trap_G2API_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, muzzleOrg);
		trap_G2API_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_X, muzzleDir);

		trap_FX_PlayEffectID(trap_FX_RegisterEffect("effects/turret/muzzle_flash.efx"), muzzleOrg, muzzleDir);
