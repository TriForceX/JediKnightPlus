...
static void CG_DrawLagometer( void ) {
	...
	if (cg.snap->ps.pm_type == PM_SPECTATOR || JKMod_CG_ShowScores()) y = cgs.screenHeight - (jkcvar_cg_drawClock.integer ? 89 : 48);
...
static void CG_DrawCrosshair( vec3_t worldPoint, int chEntValid ) {
	...
	// Tr!Force: [CGameGeneral] Don't show with movement keys at center
	if( jkcvar_cg_drawMovementKeys.integer == 2 && cg.snap->ps.pm_type != PM_SPECTATOR )
	{
		return;
	}

	// Tr!Force: [CGameGeneral] Don't show while changing map
	if (cg.mMapChange)
	{
		return;
	}
...
static void CG_DrawCrosshairNames( void ) {
	...
	if (jkcvar_cg_drawPlayerNames.integer && cg.jkmodCG.playerLabel[cg.crosshairClientNum])
	{
		return;
	}
