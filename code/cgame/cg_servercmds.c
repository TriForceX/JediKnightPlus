...
static void CG_ParseScores( void ) {
	...
	for ( i = 0 ; i < readScores ; i++ ) {
		...
		cg.scores[i].guantletCount = atoi(CG_Argv(i * scoreOffset + 13));
...
static void CG_ParseServerinfo( const char *info ) {
	...
	cgs.jkmodCGS.teamPassThrough = atoi(Info_ValueForKey(info, "jk_teamPassThrough"));	// Tr!Force: [TeamPassThrough] Main cvar
