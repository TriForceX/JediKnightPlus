...
float	UI_MachinegunSpinAngle( playerInfo_t *pi ) {
	...
	if ( pi->barrelSpinning == !(torsoAnim == BOTH_ATTACK3) ) {
		pi->barrelTime = dp_realtime;
		pi->barrelAngle = AngleMod( angle );
		pi->barrelSpinning = !!(torsoAnim == BOTH_ATTACK3);
	}
...
static qboolean UI_ParseAnimationFile( const char *filename, animation_t *animations ) {
	...
	for ( i = 0 ; i < MAX_ANIMATIONS ; i++ ) {
		...
		if ( i == BOTH_CROUCH1WALK ) {
			skip = animations[BOTH_CROUCH1WALK].firstFrame - animations[BOTH_GESTURE1].firstFrame;
		}
		if ( i >= BOTH_CROUCH1WALK ) {
...
void UI_PlayerInfo_SetInfo( playerInfo_t *pi, int legsAnim, int torsoAnim, vec3_t viewAngles, vec3_t moveAngles, weapon_t weaponNumber, qboolean chat ) {
	...
	else if ( ( currentAnim == BOTH_GESTURE1 || currentAnim == BOTH_ATTACK3 ) && ( torsoAnim != currentAnim ) ) {
		pi->pendingTorsoAnim = torsoAnim;
	}
