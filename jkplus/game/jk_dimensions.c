...
static void trap_Trace_Start(int entityNum)
{
	if (0 <= entityNum && entityNum < MAX_CLIENTS)
	{
		...
		if (g_entities[entityNum].client && (g_entities[entityNum].client->ps.stats[JK_DIMENSION] & JK_TEMP_IN)) {
			int i;
		...
		// Final check
			...
			for (i = 0; i < level.num_entities; i++) {
				if (i != entityNum) {
					if (g_entities[i].inuse && g_entities[i].client && (g_entities[i].client->ps.stats[JK_DIMENSION] & JK_TEMP_IN) ||
						((jkcvar_altDimensions.integer & (1 << DIMENSION_DUEL)) && (g_entities[i].client->ps.stats[JK_DIMENSION] & JK_DUEL_IN)) ||
